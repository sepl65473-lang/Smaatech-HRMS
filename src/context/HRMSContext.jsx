...
export function HRMSProvider({ children }) {
  ...
  const resetPassword = useCallback((email, newPassword) => authApi.resetPassword(email, newPassword), []);
  ...
  const deleteEmployee = async (id) => {
    ...
    // Unlink any login profile that pointed at this employee (Employee-role ESS access).
    const loginProfiles = settings.loginProfiles;
    if (emp && loginProfiles?.some((p) => p.empName === emp.name)) {
      await updateSettings({
        loginProfiles: loginProfiles.map((p) => (p.empName === emp.name ? { ...p, empName: '' } : p)),
      }, false);
    }
  ...
  const value = {
    isAuthenticated: Boolean(authUser), login, loginWithFace, finishLogin, logout, resetPassword,
