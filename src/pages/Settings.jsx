...
export default function Settings() {
  ...
  const users = settings.loginProfiles?.length ? settings.loginProfiles : DEFAULT_LOGIN_PROFILES;
  ...
  const saveUser = (user) => {
    const next = editingUser
      ...
      : [...users, user];
    updateSettings({ loginProfiles: next }, false);
    setUserFormOpen(false);
  };
  const removeUser = (id) => {
    updateSettings({ loginProfiles: users.filter((u) => u.id !== id) }, false);
        ...
        <div className="table-scroll">
          <table className="table">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  <td>{u.name}{u.name === currentUser.name && u.role === currentUser.role ? ' (you)' : ''}</td>
