          ...
          <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input
              ...
              onChange={(e) => {
                ...
                if (e.target.checked && !loginEmail) setLoginEmail(form.email);
              }}
            />
            <span>Create a workspace login for this employee now</span>
          </label>
          {createLogin && (
            <div className="form-grid" style={{ marginTop: 10 }}>
              <Field label="Login email" error={errors.loginEmail}>
                <input className="input" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="name@smaatech.co" />
              </Field>
