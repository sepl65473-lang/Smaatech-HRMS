          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                ...
                logs.map((log, index) => (
                  <tr key={log._id || log.id || `${log.createdAt}-${index}`} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3 px-4">
                      <div className="font-semibold text-slate-200">{log.actor || 'System'}</div>
                      <div className="text-[10px] text-slate-400">{log.role || 'Automation'}</div>
                    </td>
                    <td className="py-3 px-4">
