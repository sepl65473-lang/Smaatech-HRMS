...
6. **Seed passwords have hardcoded fallbacks.** `server/src/seed.js` falls back to `Admin@123` / `Manager@123` / `Finance@123` / `Employee@123` when the `SEED_*_PASS` variables are unset, and prints them to stdout. The same four strings are also hardcoded in `client/src/lib/permissions.js`'s `DEFAULT_LOGIN_PROFILES` and ship in the client bundle — dead weight, since (per #1 above) the `password` field on those objects is never read by any code path. **Always set `SEED_*_PASS`, delete the demo accounts after setup, and strip the `password` keys from `DEFAULT_LOGIN_PROFILES`.**
